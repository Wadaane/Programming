...
% Run Python Script
disp('Launch Python Script to acquire Images')
dos('python3 pyMat.py');

disp('Done')

% Get list of files in a directory
disp('Process each Image in Folder')
...
for i = 1 : n_files
    ...
    file_name = ['Images/Samples/', file_name];
    res = processImage(file_name);
    disp(res)
    results = results + res;
end
...
% fileName = 'Images/Latest.jpg';
% results = processImage(fileName);
disp('Done')
disp(results)

function results = processImage(fileName)
    % Acquire image
    src = imread(fileName);  % I put all images in the subfolder Images.
    % RBC: 304
    % WBC: 2
    % Platelets: 35
    src_original = src;
...
function msg = send(s, cmd)%#ok<DEFNU>
    fprintf(s, [cmd, '#']);
